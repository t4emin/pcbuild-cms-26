...
import { getProductRows, parseProduct, requireAdminRequest, slugify, validateProduct, type ProductInput } from "../../../api-helpers";
...
export async function POST(request: Request) {
  ...
  try {
    await db.prepare("INSERT INTO products (id, slug, category, brand, model, title, short_description, description, price, affiliate_url, shop_name, images, attributes, performance_score, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
      .bind(id, slug, input.category, input.brand?.trim(), input.model?.trim(), input.title?.trim(), input.shortDescription?.trim() ?? "", input.description?.trim() ?? "", Number(input.price), input.affiliateUrl?.trim() ?? "", input.shopName?.trim() ?? "", JSON.stringify(input.images ?? []), JSON.stringify(input.attributes ?? {}), Number(input.performanceScore ?? 0), input.status === "published" ? "published" : "draft", now, now).run();
  } catch (error) {
