...
import { auditRequest, normalizeAffiliateUrl, parseProduct, requireAdminRequest, slugify, validateProduct, type ProductInput } from "../../../../api-helpers";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.prepare("UPDATE products SET slug = ?, category = ?, brand = ?, model = ?, title = ?, short_description = ?, description = ?, price = ?, affiliate_url = ?, shop_name = ?, images = ?, attributes = ?, performance_score = ?, status = ?, updated_at = ? WHERE id = ?")
    .bind(slugify(input.slug || input.title || id), input.category, input.brand?.trim(), input.model?.trim(), input.title?.trim(), input.shortDescription?.trim() ?? "", input.description?.trim() ?? "", Number(input.price), normalizeAffiliateUrl(input.affiliateUrl), input.shopName?.trim() ?? "", JSON.stringify(input.images ?? []), JSON.stringify(input.attributes ?? {}), Number(input.performanceScore ?? 0), input.status === "published" ? "published" : "draft", now, id).run();
  ...
  await auditRequest(request,"update","product",id,input.title?.trim()||id);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if(result.meta.changes>0)await auditRequest(request,"delete","product",id);
